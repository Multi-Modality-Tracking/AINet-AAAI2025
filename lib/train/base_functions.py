...
from lib.train.dataset import visEvent_trainingSet,visEvent_testingSet,DMET_testingSet,DMET_trainingSet
...
def names2datasets(name_list: list, settings, image_loader, attr=None):
    ...
    for name in name_list:
        #print('name',name)
        assert name in ['LasHeR_testingSet','RGBT234',"LasHeR_trainingSet",'LasHeR','visEvent_trainingSet','visEvent_testingSet','VTUAV','VTUAVST',"DMET","DMET_test"]
        if name == "LasHeR_trainingSet" or name == 'LasHeR':
            if settings.use_lmdb:
                ...
                datasets.append(LasHeR_trainingSet(settings.env.lasher_dir, split='train', image_loader=image_loader, attr=attr))
        if name == "RGBT234":
            datasets.append(RGBT234(settings.env.rgbt234_dir, image_loader=image_loader))
        if name == "LasHeR_testingSet":
            datasets.append(LasHeR_testingSet(settings.env.lasher_dir, split='train', image_loader=image_loader, attr=attr))
        if name == "VTUAV":
            datasets.append(VTUAV(settings.env.vtuav_dir, split='train', image_loader=image_loader))
        if name == "VTUAVST":
            datasets.append(VTUAV(settings.env.vtuav_dir, split='val_st', image_loader=image_loader))
        if name == "visEvent_trainingSet":
            datasets.append(visEvent_trainingSet(settings.env.visevent_trainingset_dir, split='train', image_loader=image_loader))
        if name == "visEvent_testingSet":
            datasets.append(visEvent_testingSet(settings.env.visevent_testingset_dir, split='test', image_loader=image_loader))

        if name == "DMET":
            datasets.append(DMET_trainingSet(settings.env.dmet_dir))
        if name == "DMET_test":
            datasets.append(DMET_testingSet(settings.env.dmet_dir))
...
def get_optimizer_scheduler(net, cfg):
    ...
    # ostrack
        ...
        if is_main_process():
            print("Learnable parameters are shown below.")
            for n, p in net.named_parameters():
                # if 'backbone' in n and 'mamba' not in n:
                #     p.requires_grad=False
                # else:
                if p.requires_grad:
                    print(n)
