...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, display_name: str = None,
        ...
        if self.run_id is None:
            ...
            if self.update:
                ...
                self.results_dir = '{}/{}/{:03d}'.format(env.results_path, self.name, self.run_id)
        if result_only:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_path_v, frame_path_i in zip(seq.frames_v[1:], seq.frames_i[1:]):
            ...
            # iou = self.cal_iou(out['target_bbox'], seq.ground_truth_rect[frame_num])
            # ious.append(iou)
            prev_output = OrderedDict(out)
            _store_outputs(out, {'time': time.time() - start_time})
        
        # iou_path = '/data1/Code/luandong/WWY_code_data/Codes/sigma_fusion/tracking/iou_labels/ab_6'
        # os.makedirs(iou_path,exist_ok=True)
        # f_path = os.path.join(iou_path, seq.name+'.txt')
