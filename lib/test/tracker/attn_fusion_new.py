...
from lib.models.ostrack_attnFusion import build_ostrack_attn_fusion
...
from lib.test.tracker.async_update import AsyncUpdate

class OSTrack_twobranch(BaseTracker):
    def __init__(self, params, dataset_name):
        super(OSTrack_twobranch, self).__init__(params)
        network = build_ostrack_attn_fusion(params.cfg, training=False)
        try:
        ...
        # inner_attn_model_path = "/home/zhaojiacong/ostrack_attnFusion/output/checkpoints/train/attn_fusion/cross_attn_fusion/OSTrack_twobranch_ep0055.pth.tar"
        # checkpoint = torch.load(inner_attn_model_path, map_location="cpu")['net']
        # for k,v in list(checkpoint.items()):
        #     if "inner_attn" not in k:
        #         del checkpoint[k]
        # missing_keys, unexpected_keys = network.load_state_dict(checkpoint, strict=False)
        # print('Load inner_attn model from: ' + inner_attn_model_path)
        ...
        self.state = None
        # self.update_way = AsyncUpdate(mode="once", thr=params.update, timing=None)
        self.update_way = AsyncUpdate(mode="always", thr=params.update, timing=None)
        # self.update_way = AsyncUpdate(mode="timing", thr=params.update, timing=5)
        ...
        if self.debug or params.update[0]<1.:
            self.add_hook_inner()
    ...
    def track(self, image_v,image_i, info: dict = None):
        ...
        #         except:
        #             attn_sum = sum(blk.inner_attn.attn.attn)
        ...
        # 模板更新
        is_update = 0
        if hasattr(self, "modal_weight") and self.modal_weight!=None:
            if self.debug:
                self.visdom.register(self.modal_weight, 'lineplot', 1, 'contribution')
            is_update = self.update_way(score=response.max().item(), contr_rgb=self.modal_weight[-1][0].item(), contr_tir=self.modal_weight[-1][1].item())
            is_update, idx_reverse = is_update, None if isinstance(is_update, int) else is_update
        else:
            if self.debug:
                print("未检测到modal weight")
    ...
    def add_hook_inner(self):
        self.modal_weight_once_rgb = 0
        self.modal_weight_once_tir = 0
        self.modal_weight = None        # time_step, modal
        self.all_times = 2
        self.times = self.all_times-1
        self.model_num = 0

        def note_modal_weight_once(this, input, output):
            attn_mean = output[1]
            length = attn_mean.shape[-2]
            # self.modal_weight_once_rgb.append(attn_mean[..., :length].sum(-1).mean())
            # self.modal_weight_once_tir.append(attn_mean[..., length:].sum(-1).mean())
            self.modal_weight_once_rgb += attn_mean[..., :length].sum()/length
            self.modal_weight_once_tir += attn_mean[..., length:].sum()/length
            self.times = (self.times+1)%self.all_times

        def note_modal_weight(this, input, output):
            attn_mean = output[1]
            length = attn_mean.shape[-2]
            # self.modal_weight_once_rgb.append(attn_mean[..., :length].sum(-1).mean())
            # self.modal_weight_once_tir.append(attn_mean[..., length:].sum(-1).mean())
            ...
            # 有的模型会跑两次，所以再确认一下
            if self.times==(self.all_times-1):
                a = torch.tensor([self.modal_weight_once_rgb, self.modal_weight_once_tir]).cuda()
                a /= self.model_num
                a = a.softmax(dim=0).unsqueeze(0)
                # a = a.mean(-1)/12    # 标准方法【12是头的数量】
                # a = (a*12).softmax(-1)  # 缩放【12是缩放因子】
                if self.debug:
                    self.modal_weight = a if self.modal_weight==None else torch.cat([self.modal_weight[-200:,:], a], dim=0)
                else:
                    self.modal_weight = a
                self.modal_weight_once_rgb = 0
                self.modal_weight_once_tir = 0

        for i,blk in enumerate(self.network.backbone.blocks):
            if hasattr(blk, 'inner_attn'):
                self.model_num+=1
                if hasattr(blk.inner_attn, 'inner_attn'):
                    if i==(len(self.network.backbone.blocks)-1):
                        blk.inner_attn.inner_attn.register_forward_hook(note_modal_weight)
                    else:
                        blk.inner_attn.inner_attn.register_forward_hook(note_modal_weight_once)
                elif hasattr(blk.inner_attn, 'attn'):
                    if i==(len(self.network.backbone.blocks)-1):
                        blk.inner_attn.attn.register_forward_hook(note_modal_weight)
                    else:
                        blk.inner_attn.attn.register_forward_hook(note_modal_weight_once)
